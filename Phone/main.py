def action(phonebook):
    while True:
        user_choice = input('1 - Импортировать контакт\n2 - Найти контакт\n3 - Добавить контакт\n4 - Изменить контакт\n5 - Удалить контакт\n6 - Все контакты\n0 - Выход')
        print()
        if user_choice == '1':
            add_file = input ('Введите название контакта: ')
            import_data(add_file, phonebook)
        elif user_choice == '2':
...
def import_data(add_file,phonebook):
    try:
        with open(add_file, 'r', encoding='utf-8') as new_contact, open(phonebook, 'a', encoding='utf-8') as file:
            add_contact = new_contact.readlines()
